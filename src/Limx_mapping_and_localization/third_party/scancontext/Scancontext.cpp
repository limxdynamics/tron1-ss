...
std::pair<int, float> SCManager::detectLoopClosureIDGivenScan ( const pcl::PointCloud<SCPointType> & _scan_down  )
{
    ...
    // 新增变量存储次小距离
    double second_min_dist = 10000000; // 初始化为极大值
    ...
    for ( int candidate_iter_idx = 0; candidate_iter_idx < NUM_CANDIDATES_FROM_TREE; candidate_iter_idx++ )
    {
        ...
        if( candidate_dist < min_dist )
        {
            ...
            second_nn_align = nn_align;
            second_nn_idx = nn_idx;

            min_dist = candidate_dist;
            nn_align = candidate_align;

            nn_idx = candidate_indexes[candidate_iter_idx];
        }else if (candidate_dist < second_min_dist) {
            second_min_dist = candidate_dist;
            second_nn_align = candidate_align;
            second_nn_idx = candidate_indexes[candidate_iter_idx];
        }

        if(candidate_dist < SC_DIST_THRES){
            keyframe_ids.insert(candidate_indexes[candidate_iter_idx]);
        }
    }

    // 计算距离比并判断可靠性
    double distance_ratio = (second_min_dist > 0) ? (min_dist / second_min_dist) : 1.0; // 避免除以零
    const double RATIO_THRESHOLD = 0.8;
    std::cout<<"min_dist = "<<min_dist<<" ; second_min_dist = "<<second_min_dist<<std::endl;

    // if(keyframe_ids.size() > 2){
    //     std::cout<<"nn_idx = "<<*keyframe_ids.begin()<<" ; second_nn_idx = "<<*keyframe_ids.rbegin()<<std::endl;
    //     // if(std::fabs(*keyframe_ids.begin() - *keyframe_ids.rbegin() ) > 8)
    //     // {
    //     //     nn_idx = -1;
    //     // }
    //     if(std::fabs(nn_idx - second_nn_idx ) < 5){
    //         nn_idx = -1;
    //     }
    // }
    ...
    //     loop_id = nn_idx;
    //     std::cout << "Reliable loop closure found. Distance ratio: " << distance_ratio << std::endl;
    // } else {
    //     loop_id = -1;
    //     std::cout << "Ambiguous loop closure. Distance ratio: " << distance_ratio << std::endl;
    // }
